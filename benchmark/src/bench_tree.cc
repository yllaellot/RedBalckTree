...
{
    bencht::run_bench_set(state, "bench15");
}
BENCHMARK(BM_Set_2)->Unit(benchmark::kMillisecond);

static void BM_RBT_2(benchmark::State& state)
{
    bencht::run_bench_rbt(state, "bench15");
}
BENCHMARK(BM_RBT_2)->Unit(benchmark::kMillisecond);

static void BM_Set_3(benchmark::State& state)
{
    bencht::run_bench_set(state, "bench16");
}
BENCHMARK(BM_Set_3)->Unit(benchmark::kMillisecond);

static void BM_RBT_3(benchmark::State& state)
{
    bencht::run_bench_rbt(state, "bench16");
}
BENCHMARK(BM_RBT_3)->Unit(benchmark::kMillisecond);
